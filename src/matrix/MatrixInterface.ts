...
export class MatrixInterface {
  ...
  public async sendKick(bridge: Bridge, mcKick: MCEvents.Kick): Promise<void> {
    ...
    // Perform the kick
    client.kickUser(this.appservice.getUserIdForSuffix(uuid), bridge.room, mcKick.message);
  }
  ...
  private async onMxLeave(room: string, event: any): Promise<void> {
    // Check if the event hasn't already been redacted
    const content: any | undefined = event['content'];
    const old_content: any | undefined = event['prev_content'];
    ...
    const state_key: string = event['state_key'];
    if (!this.appservice.isNamespacedUser(state_key))
      return;

    const membership: string = content['membership'];
    const oldMembership: string = old_content['membership'] || "leave";
